//! WMA 基准测速（Rust 侧，零依赖）。/ WMA benchmark (Rust side, dependency-free).
//!
//! 运行 / Run:
//! ```text
//! cargo bench --bench wma_bench
//! ```
//! 对照原生 C（需系统安装 TA-Lib C 库）:
//! ```text
//! cargo bench --bench wma_bench --features bench-c
//! ```
...
fn main() {
    ...
    for _ in 0..ITERS {
        let out = wma(&prices, PERIOD).unwrap();
        checksum += out[out.len() - 1];
    }
    let elapsed = start.elapsed();
    println!("Rust WMA:  {ITERS} iters x {N} elems = {elapsed:?}");
...
fn run_c_bench(prices: &[f64]) {
    unsafe {
        ...
        for _ in 0..ITERS {
            ...
            assert_eq!(rc, 0, "TA_WMA failed");
            checksum += out[(nb - 1) as usize];
        }
        let elapsed = start.elapsed();
        println!("C WMA (native): {ITERS} iters x {N} elems = {elapsed:?}");
        println!("  avg/call: {:?}", elapsed / ITERS as u32);
        println!("  ns/elem : {:.2}", elapsed.as_nanos() as f64 / ITERS as f64 / N as f64);
        println!("  checksum (anti-optimize): {checksum}");
        TA_Shutdown();
