...
## 战略决策（用户确认，2026-08-10，待执行 / recorded before execution）
...
- **执行顺序**：P0（已完成）→ **P1 测量底座** → **P2 共享原语优化**（在 65 上验证，黄金向量 + ns/elem 护栏）→ **P4 在高速原语上实现剩余 96**（P4-0 先补本版类别内 4 个，再 Cycle / 数学 / 模式）。
- 本决策已记录；待用户发出执行指令后，再启动 P0-3 / P0-4 收尾与 P1。
...
### P2-1 嵌套 EMA 融合核（DEMA / TEMA / T3）【最高收益】
...
- **验收**：黄金向量 1:1 通过；bench ns/elem 较 P1 基线显著下降。
...
- 本机**已安装** TA-Lib C + PyPI `TA-Lib` 0.7.1（P0-1 完成；`generate.py` 已成功跑通、63 fixture 重生成、权威黄金向量护栏就绪）→ P0-2 已完成，P0-3/P0-4 收尾中。
