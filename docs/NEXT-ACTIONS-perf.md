...
| **D 蜡烛决策分支**（已转换模板） | `cdl_engulfing` 1.996 / `cdl_separatinglines` 1.743 / `cdl_harami` 1.632 / `cdl_longline` 1.296 / `cdl_shortline` 1.307 | 1.3–2.0× | 每 bar 分支 + CandleAvg 滑动窗口 | **高**：每 bar 仅依赖 CandleAvg 窗口，分块 + 前 `avgPeriod` 重叠播种 |
...
- 并行阶段 KPI：消除全部 <1× 伪慢（即全量 ≥1×），并对可并行子集（A/C/D 类 + `adosc`）实现 Rust/C < 1（多核超 C 单线程）；`ht_*` 因长记忆递推，并行收益最末、风险最高，可置后。
